...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to image directory (ex. ./AGD20K)",
)
parser.add_argument(
    ...
    for obj in tqdm(os.listdir(f"{input_path}/{aff}"), desc="Object"):
        ...
        for img in img_list:
            ...
            hand_mask = cv2.imread(
                f"{mask_path}/{aff}/{obj}/{IMG_NAME}_mask_{text_prompt}.png",
            ...
            cv2.imwrite(
                f"{output_path}/{aff}/{obj}/{IMG_NAME}_removed_{text_prompt}.png",
                masked_image,
            )

            # system_prompt = (
            #     f"Draw on a white background, following the existing {obj}."
            #     "Draw only one {obj}, and absolutely nothing else."
            # )
            system_prompt = (
                f"Just draw the {obj} floating in the air, and wall, background."
                f"Do not draw human or human hand. Just draw the {obj}."
                f"The object should be at the front."
            )
            ...
            response = client.images.edit(
                ...
                # prompt=f"A picture of the {obj}" + system_prompt,
                prompt=prompt,
                n=1,
                size=image_size,
            ).data[0]
            ...
            urllib.request.urlretrieve(
                response.url,
                f"{output_path}/{aff}/{obj}/{IMG_NAME}_nohand_{text_prompt}.png",
            )
